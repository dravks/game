...
- The class visual consistency pass has been completed: character creation, city, overworld, and dungeon player spawning now prefer the `class and cat` directional class images, with the old player spritesheets kept only as fallback assets.
- The pet/chest/effect bugfix pass has been completed: Loot Cat now renders above the player and can be summoned in dungeons, dungeon mob gold drops are real pickups for the pet, one-shot attack effects are forcibly cleaned up, and victory chest opening is gated behind an active nearby post-boss chest instead of loose Enter/Alt+Enter state.
- The overworld pet input follow-up has been completed: Amasra now has a clickable `[P] Loot Cat` HUD toggle, direct `P` key binding, duplicate-toggle guard, and pet update continues even when overlay state blocks movement.
- The first Maradon-road overworld expansion pass has been completed: Amasra Outskirts is now a larger 4600x2400 field using Tiny Swords terrain/building/resource assets, with a west city gate, stone road, outpost cluster, lumber camp, old mine, river edge, forest belts, larger slime zones, relocated King Slime nest, and a clearer field hotbar frame.
- The second overworld polish pass has been completed: the field now has a fenced safe-zone boundary, old river bridge, hand-built mine entrance landmark, clearer level-zone markers, and colored minimap bands for gate grass, wet meadow, and slime nest progression.
- The overworld collision/readability pass has been completed: visible non-walkable objects now have matching static blockers for fences, gate posts, buildings, bridge rails, river banks, mine entrance, dense forest masses, and nest rock walls while keeping the main road and bridge route open.
- The first overworld slot/reward pass has been completed: Amasra now has explicit farm slots for Gate Grass, Goblin Camp, Wolf Woods, Mine Mouth, and King Slime Nest, with per-slot levels, drop tiers, gold/XP, materials, class gear drops, and Loot Cat-compatible field pickups.
- The class equipment progression pass has been completed: every class now generates 10 deterministic tiers for each equipment slot, from white starter gear through blue, yellow, and orange Krowaz-style tiers; higher tiers require the class primary stat and orange armor tiers activate class set bonuses with real stat effects.
- The overworld junk-token cleanup has been completed: normal field mobs no longer drop placeholder camp/slot tokens that only fill inventory space; field loot now focuses on gold, class gear, and special mini-boss materials.
- The first stabilization/refactor pass has been completed: legacy hotbar calls now route safely into the current hotbar system, legacy single-quest state migrates into the newer quest-state map, missing character-panel stat aliases are covered, overworld farm-slot data has been moved into `OverworldFieldData`, field drop balance values are centralized, and non-slime field mobs now get lightweight family silhouettes.
- The first centralized item database pass has been completed: `ItemDatabase` now owns rarity definitions, set bonus definitions, 10-tier class gear generation, class gear labels/icons, and item template lookup, while `GameState` keeps the existing runtime inventory/equip/stat APIs by syncing from the database.
- The second data-split pass has been completed: upgrade/difficulty/XP/drop balance values now live in `BalanceConfig`, quest definitions now live in `QuestDatabase`, `GameplayExtensions` no longer generates fallback equipment, runtime item lookup routes through `ItemDatabase`, and legacy item alias support was intentionally removed so new test builds use the clean item ID schema only.
- The skill/class data split pass has been completed: class skills now live in `SkillDatabase`, class stat multipliers and basic attack range/projectile profiles now live in `ClassBalanceConfig`, `GameState` no longer stores duplicated class skill definitions, and overworld/dungeon basic attacks now read the same centralized combat profile.
- The first MMO foundation pass has been started: economy sinks now live in `EconomyConfig`, dungeon clear grading/boss phase data now lives in `DungeonRewardConfig`, social/PvP starter rules now live in `SocialPvpConfig`, `GameState` exposes repair/entry-fee/market-tax/reward-score/party/guild/PvP APIs, dungeon gates charge difficulty entry fees, dungeon chests display clear grade reward planning, and overworld kills now emit quest/activity events.
- The first MMO foundation UI hookup has been completed: the city exposes `MMO Systems` prototype actions for party/guild/PvP/market-tax testing, the anvil panel exposes upgrade actions for equipment, dungeon return activity feed shows clear grades, and the service flow now makes the new economy/social/PvP APIs visible for manual testing.
- The MMO systems/repair correction pass has been completed: the `MMO Systems` NPC has been removed from town and replaced with a HUD button for prototype-only social/economy tools, repair has moved into Sundries service flow instead of the anvil, equipment now gains max/current durability when created or equipped, item tooltips show durability, field/dungeon attacks wear equipped weapons, and broken or near-broken weapons now reduce effective attack power until repaired.
- The Sundries/MMO visibility follow-up has been completed: Sundries now exposes a clear `Repair Equipped Gear` button even before individual repair rows, repair cost scales with missing durability and is free when gear is already full, and the MMO HUD button plus panel now show actual Party/Guild/PvP/Economy/Durability overview content instead of feeling like an empty NPC replacement.
- The MMO Systems submenu follow-up has been completed: the HUD button now opens a real hub with clickable Party, Guild, PvP Arena, and Economy sections, each section has its own back navigation and actionable prototype rows instead of only one-line banner previews.
- The runtime NPC/service patch mismatch has been fixed: `NpcSystemsPatchV2` now routes `mmo_systems` into the MMO submenu panel and injects `Repair Equipped Gear` plus individual durability repair rows into the actual Sundries panel that runs in-game.
- The full-repair and MMO state follow-up has been completed: Sundries now exposes one `Full Repair` action that repairs all equipped and inventory gear instead of listing every item, and MMO Systems submenu actions now mutate prototype state for party members/loot mode, guild bank/notice, PvP queue/rewards, and economy snapshots so those systems have real behavior to expand later.
- The first mercenary companion pass has been completed: MMO Systems > Party can now hire/dismiss 1-hour Warrior/Rogue/Mage/Archer mercenaries, the contract persists in `mercenaryState` with 35% loot cut and 1.45x slower attack cadence, field and dungeon scenes spawn the hired class companion, the mercenary follows the player across maps, attacks nearby enemies with class-appropriate range/projectile behavior, and gold/item drops now respect the mercenary cut/claim logic.
- The pet/companion texture-swap crash has been fixed: overworld and dungeon player/pet/mercenary texture changes now go through guarded `safeSetTexture` helpers so missing or partially loaded directional texture keys no longer crash Phaser during movement.
- A temporary city test NPC named `Çakır` has been added with the archer class visual; it is isolated in `NpcManager.createNpcLayer()` as a single removable line for later cleanup.
- The temporary `Çakır` city NPC now patrols between a small set of city waypoints, updating its archer directional texture, label, shadow, and interaction hit area while moving.
